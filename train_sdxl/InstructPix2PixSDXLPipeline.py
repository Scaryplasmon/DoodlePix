"""
    ...
    https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/stable_diffusion/pipeline_stable_diffusion_xl_instruct_pix2pix.py
...
class StableDiffusionXLInstructPix2PixPipeline(
    ...
    Args:
        ...
        fidelity_mlp:
            FidelityMLP model for fidelity-based modifications.
    """
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # Same for prompt_2 if provided
            if prompt_2 is not None:
                # Similar pattern matching for prompt_2 (optional, can be implemented if needed)
                pass
            ...
            # Get fidelity embedding and adjust target dimension if needed
            target_dim = prompt_embeds.shape[-1]
            fidelity_embedding = self.fidelity_mlp(fidelity_tensor, target_dim=target_dim)
            
            # Modify the first token embedding
            prompt_embeds[:, 0] = prompt_embeds[:, 0] + 0.2 * fidelity_embedding
            
            # For SDXL we also have pooled_prompt_embeds that might need similar treatment
            if pooled_prompt_embeds is not None:
                # Apply similar modification to pooled embeddings if needed
                pass
        ...
        if do_classifier_free_guidance:
            # Apply same fidelity modifications to negative prompt embeds if needed
            if self.fidelity_mlp is not None and negative_prompt_embeds is not None:
                ...
                target_dim = negative_prompt_embeds.shape[-1]
                fidelity_embedding = self.fidelity_mlp(fidelity_tensor, target_dim=target_dim)
                
                # Modify the first token of negative embedding
