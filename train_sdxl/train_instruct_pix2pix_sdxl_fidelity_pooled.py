...
def parse_args():
    ...
    parser.add_argument(
        "--inject_fidelity_to_pooled",
        action="store_true",
        help="Inject the fidelity embedding into the pooled text embeddings (add_text_embeds) in addition to the sequence embeddings.",
    )
    parser.add_argument(
        "--fidelity_loss_type",
        type=str,
...
def main():
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad(): # Text encoders are frozen
                    for idx, (tokenizer, text_encoder) in enumerate(zip(tokenizers, text_encoders)):
                        ...
                        # --- START: Sequence Injection (Modified) ---
                        target_seq_dim = current_prompt_embeds.shape[-1]
                        with torch.no_grad(): # MLP doesn't need gradients
                            fidelity_embedding_adjusted_seq = current_fidelity_mlp(fidelity_tensor, target_dim=target_seq_dim)


                        # Ensure dtype matches before adding
                        fidelity_embedding_adjusted_seq = fidelity_embedding_adjusted_seq.to(dtype=current_prompt_embeds.dtype)

                        # Add to the first token embedding ([CLS] or BOS)
                        current_prompt_embeds[:, 0:1, :] = current_prompt_embeds[:, 0:1, :] + injection_scale * fidelity_embedding_adjusted_seq.unsqueeze(1)
                        # --- END: Sequence Injection (Modified) ---

                        prompt_embeds_list.append(current_prompt_embeds)

                       
                # --- START: Pooled Injection (NEW) ---
                if args.inject_fidelity_to_pooled and pooled_prompt_embeds is not None:
                    logger.info(f"Injecting fidelity into pooled embeddings (dim: {pooled_prompt_embeds.shape[-1]})") # Log info
                    target_pooled_dim = pooled_prompt_embeds.shape[-1]
                    with torch.no_grad(): # MLP doesn't need gradients
                        fidelity_embedding_adjusted_pooled = current_fidelity_mlp(fidelity_tensor, target_dim=target_pooled_dim)

                    # Ensure dtype matches before adding
                    fidelity_embedding_adjusted_pooled = fidelity_embedding_adjusted_pooled.to(dtype=pooled_prompt_embeds.dtype)

                    # Add the adjusted fidelity embedding to the pooled embeddings
                    # pooled_prompt_embeds shape: [B, pooled_dim]
                    # fidelity_embedding_adjusted_pooled shape: [B, pooled_dim]
                    pooled_prompt_embeds = pooled_prompt_embeds + injection_scale * fidelity_embedding_adjusted_pooled
                elif args.inject_fidelity_to_pooled and pooled_prompt_embeds is None:
                        logger.warning("Flag --inject_fidelity_to_pooled was set, but pooled_prompt_embeds could not be obtained. Skipping pooled injection.")
                # --- END: Pooled Injection ---
    ...
    if accelerator.is_main_process:
        ...
        unwrapped_fidelity_mlp = unwrap_model(fidelity_mlp) if fidelity_mlp is not None else None
        ...
        if unwrapped_fidelity_mlp is not None:
             pipeline.fidelity_mlp = unwrapped_fidelity_mlp.to(pipeline.device, dtype=pipeline.unet.dtype)
             
        pipeline.config.inject_fidelity_to_pooled = args.inject_fidelity_to_pooled
