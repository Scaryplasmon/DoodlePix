...
class StableDiffusionXLInstructPix2PixPipeline(
    ...
    def __init__(
        ...
        if fidelity_mlp is not None:
            ...
            print("are you sure you want to do this?---You're not passing a fidelity MLP")
    ...
    def encode_prompt(
        ...
        # ----- START FIDELITY EXTRACTION -----
        fidelity_val = 0.5 # Default fidelity
        clean_prompt = prompt # Keep original prompt structure if no fidelity MLP
        clean_prompt_2 = prompt_2

        if self.fidelity_mlp is not None and prompt_embeds is None: # Only extract if MLP exists and not using precomputed embeds
            if isinstance(prompt, str):
                # Regex to find "fX," at the start, allowing spaces
                match = re.match(r"^\s*f\s*(\d+),?\s*", prompt, re.IGNORECASE)
                if match:
                    f_int = int(match.group(1))
                    f_int = max(0, min(f_int, 9)) # Clamp to 0-9 range
                    fidelity_val = f_int / 9.0 # Normalize to [0.0, 1.0]
                    # Remove the fidelity prefix for tokenization
                    clean_prompt = prompt[match.end():].strip()
                    # Assume prompt_2 uses the same fidelity if not provided explicitly with fX
                    if prompt_2 is not None:
                        match_2 = re.match(r"^\s*f\s*(\d+),?\s*", prompt_2, re.IGNORECASE)
                        if match_2:
                            # If prompt_2 has its own fX, use it (though typically they'd share)
                            f_int_2 = int(match_2.group(1))
                            f_int_2 = max(0, min(f_int_2, 9))
                            fidelity_val = f_int_2 / 9.0 # Overwrite with prompt_2's value if present
                            clean_prompt_2 = prompt_2[match_2.end():].strip()
                        else:
                            clean_prompt_2 = prompt_2 # Use original prompt_2 if no fX prefix
                    else:
                        clean_prompt_2 = clean_prompt # Use cleaned prompt if prompt_2 was None
                else:
                    # If no fX prefix found in prompt, use default fidelity and original prompts
                    clean_prompt_2 = prompt_2 or prompt # Handle None case for prompt_2
                    logger.info(f"No 'fX,' prefix found in prompt '{prompt[:20]}...', using default fidelity {fidelity_val}")


            elif isinstance(prompt, list):
                f_vals = []
                clean_prompt_list = []
                clean_prompt_2_list = [] if prompt_2 is not None else None # Initialize if prompt_2 is a list

                for idx, p in enumerate(prompt):
                    match = re.match(r"^\s*f\s*(\d+),?\s*", p, re.IGNORECASE)
                    current_f_val = 0.5 # Default for this item
                    current_clean_p = p
                    current_clean_p2 = None

                    if match:
                        f_int = int(match.group(1))
                        f_int = max(0, min(f_int, 9))
                        current_f_val = f_int / 9.0
                        current_clean_p = p[match.end():].strip()
                    else:
                        logger.info(f"No 'fX,' prefix found in prompt item '{p[:20]}...', using default fidelity {current_f_val}")

                    f_vals.append(current_f_val)
                    clean_prompt_list.append(current_clean_p)

                    # Handle prompt_2 list
                    if prompt_2 is not None:
                        p2 = prompt_2[idx]
                        match_2 = re.match(r"^\s*f\s*(\d+),?\s*", p2, re.IGNORECASE)
                        if match_2:
                            # If p2 has fX, potentially override f_val for this item and clean p2
                            f_int_2 = int(match_2.group(1))
                            f_int_2 = max(0, min(f_int_2, 9))
                            f_vals[-1] = f_int_2 / 9.0 # Override last added f_val
                            current_clean_p2 = p2[match_2.end():].strip()
                        else:
                            current_clean_p2 = p2 # Use original p2
                        clean_prompt_2_list.append(current_clean_p2)
                    else:
                        # If prompt_2 is None overall, use the cleaned prompt
                        current_clean_p2 = current_clean_p

                # Use the average fidelity across the batch if specified per item
                if f_vals:
                    fidelity_val = torch.tensor(f_vals, device=device).view(-1, 1) # Keep batch dim
                else:
                    fidelity_val = 0.5 # Fallback if somehow f_vals is empty
                clean_prompt = clean_prompt_list
                clean_prompt_2 = clean_prompt_2_list if prompt_2 is not None else clean_prompt

            # Ensure fidelity_val is a tensor for MLP input
            if isinstance(fidelity_val, float):
                fidelity_tensor = torch.full((batch_size, 1), fidelity_val, device=device)
            else: # Already a tensor for list input case
                fidelity_tensor = fidelity_val.to(device)
    ...
    def from_pretrained_with_fidelity(cls, pretrained_model_path, fidelity_mlp_path=None, **kwargs):
        """Load the pipeline with an optional fidelity MLP model."""

        # Need to pop fidelity_mlp_path if it's passed via kwargs to avoid issues with from_pretrained
        _ = kwargs.pop("fidelity_mlp_path", None)

        # Load the main pipeline components
        pipeline = cls.from_pretrained(pretrained_model_path, **kwargs)

        fidelity_mlp_instance = None
        if fidelity_mlp_path is not None:
            # Dynamically import FidelityMLP - assumes fidelity_mlp.py is accessible
            try:
                from fidelity_mlp import FidelityMLP
            except ImportError:
                raise ImportError("Could not import FidelityMLP. Make sure fidelity_mlp.py is in your Python path.")

            # Determine a hidden size for potential initialization
            # Using text_encoder_2's hidden size as it's typically larger in SDXL
            if pipeline.text_encoder_2 is not None:
                default_hidden_size = pipeline.text_encoder_2.config.hidden_size
            elif pipeline.text_encoder is not None:
                default_hidden_size = pipeline.text_encoder.config.hidden_size
            else:
                default_hidden_size = 768 # A common default
                logger.warning("Could not determine text encoder hidden size, defaulting FidelityMLP hidden size to 768")


            if os.path.exists(fidelity_mlp_path):
                logger.info(f"Loading Fidelity MLP from: {fidelity_mlp_path}")
                fidelity_mlp_instance = FidelityMLP.from_pretrained(fidelity_mlp_path)
            else:
                # Create a new one if path doesn't exist
                logger.warning(
                    f"Fidelity MLP not found at {fidelity_mlp_path}, "
                    f"initializing a new one with hidden size {default_hidden_size}"
                )
                fidelity_mlp_instance = FidelityMLP(hidden_size=default_hidden_size)

            # Move to the same device and dtype as the pipeline's UNet (a good proxy)
            fidelity_mlp_instance.to(pipeline.unet.device, dtype=pipeline.unet.dtype)
            pipeline.fidelity_mlp = fidelity_mlp_instance # Assign to the pipeline instance
