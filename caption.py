...
def get_structured_description(image, existing_attributes=None):
    ...
    # Get main subject and world
    main_subject = model.query(image, "Name the subject of this icon. max 3 words.")["answer"].lower()
    world = analyze_world_theme(image)
    
    # Analyze style, details, and colors
    style_type = analyze_style(image)
    detail_level = analyze_detail_level(image)
    color_info = analyze_image_colors(image, n_colors=3)
    
    # Get combined color and material description
    color_material_desc = get_color_material_description(image, color_info)
    
    # Get concise description focusing on unique features
    description = model.query(
        image, 
        "Describe the unique visual features of this icon in one short sentence."
    )["answer"].lower()

    # Format the final prompt with separate style and detail indicators
    prompt = (
        f"<s:{main_subject}>, "
        f"<c:{color_material_desc}>, "
        f"<w:{world} UI icon>, "
        f"<k:{detail_level}>, "
        f"<p:{style_type}>, "
        f"<d:{description}>"
